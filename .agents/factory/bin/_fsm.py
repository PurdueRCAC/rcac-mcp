...
"""Shared helpers for the cluster-mcp software-factory FSM scripts.
...
Requires PyYAML, which is pinned in the project's ``dev`` dependency group for
exactly this reason; the scripts are meant to be run as
``uv run python .agents/factory/bin/<script>.py``.
"""
