...
The ``cm-build`` and ``cm-review`` skills call this to advance state rather than
...
Usage examples:
    ...
    uv run python .agents/factory/bin/set_phase.py spec/<slug>/TECH.md \
        --add-phase P7 --name "F1 remediation: per-user context cache key" \
        --satisfies R6 --depends-on P6 --after P6 \
        --verify "uv run pytest -v -m unit -k context_cache" \
        --current P7 --top-status in_progress --touch

    # retune a reopened phase's body in place (remediation tightening a too-weak gate)
    uv run python .agents/factory/bin/set_phase.py spec/<slug>/TECH.md \
        --phase P3 --verify "uv run pytest -v -k delegate" --touch
