# The cluster-mcp software factory — methodology

This is the reference an agent reads when it needs to understand the spec-driven development
lifecycle that the `cm-*` skills implement. The skills themselves are thin; this is the *why*.
When something here disagrees with a skill, **the skill body is the operating procedure** — fix
this file.

> **Provenance.** This factory was ported from the sibling
> [HyperShell](https://github.com/hypershell/hypershell) project's `.agents/` harness, whose
> `harness-log.md` records two dozen refinements this port inherits for free. The *method* is
> shared; the invariants, the verify commands, and the blast-radius map are this project's own.
> A general-audience onboarding page (`getting-started.html`) exists upstream and has **not** been
> ported yet — see the `factory-onboarding-page` seed in `ROADMAP.md`.

## The lifecycle

One feature (or fix/refactor) flows through five lifecycle skills, on its own git branch, with every
artifact committed under `spec/{slug}/`:

```
main ──/cm-feature──▶ feature|fix/{slug}     GOAL.md            (shape: what & why, locked)
          │
          ├──────────/cm-plan──────────▶     research/ PLAN.md TECH.md   (design + phased FSM)
          │
          ├──────────/cm-build─────────▶     source + docs + instructions  (execute one phase/loop)
          │            ▲        │
          │            └────────┘  (loop until TECH.md is done)
          │
          ├──────────/cm-review────────▶     REVIEW.md          (adversarial QA, clean context)
          │            │
          │      changes-requested ──▶ back to /cm-build ;  approved ──▶
          │
          └──────────/cm-publish───────▶     PR → main   (default)  |  local squash-merge
```
...
1. **`AGENTS.md` is the constitution.** We do not invent a `constitution.md`; the skills reference
   `AGENTS.md` and the curated [`invariants.md`](invariants.md). The `cm-plan` invariant gate and the
   `cm-review` footgun checklist both draw from it.
2. **Files + git are the durable substrate.** State lives only in the committed `TECH.md`
   ...
   script). Never rely on conversation memory to carry lifecycle state — `cm-review` runs in a
   separate context and `cm-build` may run days later on another machine.
3. **Parallelism for research, never for building.** `cm-plan` fans out read-only research
   subagents (big wins). `cm-build` is strictly single-threaded and linear — parallel builders make
...
5. **Ceremony scales to appetite.** The single biggest anti-pattern in spec-driven tooling is
...
7. **Observe cheaply, act deliberately.** The factory improves *itself* through an asymmetric loop:
   ...
   (silence by default), but fixes are applied only by the human-gated `cm-harness`, with fresh eyes
...
- **Scope hammering** — nice-to-haves are cuttable; `cm-review` scope-checks against the appetite.
...
bugs can wait. That is **false** for this project's load-bearing invariants. This server executes
arbitrary shell commands, and in `delegate` mode it does so **as other people**, on shared HPC
infrastructure, from a privileged process. A bug in the executor-isolation path
([`invariants.md`](invariants.md) §1) is not a defect that can wait for the next cycle — it is one
user running commands as another. Neither is an auth minimum, an identity-claim precedence, or a
sudo argv. The `hammerable: false` phase flag operationalizes this — `cm-review` must never
scope-hammer a correctness/security phase to "fit the appetite."

## Where things live

```
.agents/
  skills/cm-{feature,plan,build,review,publish}/SKILL.md   # the five lifecycle skills
  skills/cm-harness/SKILL.md                               # meta/maintenance: apply the self-improvement loop
  skills/cm-roadmap/SKILL.md                               # operational: retire landed seeds, keep ROADMAP.md true
  skills/cm-release/SKILL.md                               # operational: cut a version
  factory/
    ...
    bin/                  # next_phase.py, set_phase.py, _fsm.py (FSM); meta_status.py (META.md reader);
                          #   sandbox.sh (throwaway HOME + fail-closed SSH for CLI drives)
    harness-log.md        # cm-harness decision ledger (cross-job anti-thrash memory)
...
*candidate*: `/cm-feature` promotes it into a real `GOAL.md`, and that promotion is where a human
negotiates appetite, non-goals and the R-IDs `cm-review` will grade. Two statuses precede promotion
— `unshaped` (a raw deferral) and `shaped` (already negotiated, but not yet accepted into a cycle) —
and neither is a shortcut past the human gate. When the cycle lands on `main`, `/cm-roadmap`
...
plan; the fixes are public when they ship, the standing inventory is not. This project is a
multi-user command-execution service on shared research infrastructure — the lane is not theoretical.
See `AGENTS.md`.
...
## The self-improvement loop (META.md + `cm-harness`)
...
```
cm-feature ─┐
cm-plan     ├─ meta-note (silence by default) ─► spec/{slug}/META.md   (What worked well + F# findings)
cm-build    │                                          │  (kept OUT of the blind reviewer's context)
cm-review  ─┘  (orchestrator only)                     │
                                                       ▼
cm-publish ──► reads open findings (meta_status.py) ─► "🔧 Harness feedback" block in the PR
                                                       ▼
cm-harness ──► human-gated: shape → preview → apply to .agents/ ─► flip status + log harness-log.md
```
...
- **Producers** (`cm-feature`/`cm-plan`/`cm-build`/`cm-review`) only *record* (≤3 terse findings each),
  never fix. `cm-build` is the richest source (per-phase, across separate invocations — appending to a
  file preserves signal a context reset erases). `cm-review`'s finding is written by the *orchestrator*,
...
- **The applier is separated from the observer** (fresh eyes + a human gate, mirroring blind review).
  `cm-harness` is **human-gated always** and bound by hard guardrails: it **never auto-weakens a
  non-negotiable gate** (tests, the sandboxed CLI-verify, an `invariants.md` item) without an explicit
  typed override — *a finding arguing to loosen a guardrail is itself a warning sign* — prefers an
  example over a new hard rule (fixes must generalize), keeps per-finding **atomic revertable** commits
  with **post-apply verification**, writes **no `META.md` findings** (no meta-on-meta, no recursion),
  and reads `harness-log.md` first so a fix that reverts a recent change or repeats a rejected one is
  flagged, not silently re-applied.
...
(via `cm-harness --all` and the ledger) escalates itself. As with the FSM, the fragile parsing is owned
