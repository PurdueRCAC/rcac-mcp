...
The `cm-*` skills are plain markdown + portable shell (`git`, `uv run`, the `.agents/factory/bin`
...
| **`Agent` subagent fan-out** (`cm-plan` research, `cm-review` reviewer) | Parallel read-only workers | **Do the work sequentially yourself**, producing the same artifacts (`research/NN-*.md`; the review). For `cm-review` this weakens *blindness* — compensate by starting a clean context and grading strictly on executed evidence (the rubric says how). |
| **`ReportFindings`** (`cm-review`) | Renders findings in the host UI | **Additive, not load-bearing** — `REVIEW.md` is the durable record. Skip the call; still write `REVIEW.md`. |
| **`Skill` / `/cm-*` launch** | How a skill starts | Launch by your harness's mechanism; the lifecycle handoffs ("then run `/cm-plan`") are advisory prose. |
...
`git`, `uv run …` (incl. the FSM scripts and `meta_status.py`), `.agents/factory/bin/sandbox.sh`
(POSIX `sh`, `mktemp`, `trap`), file read/edit/grep/glob, and every artifact under `spec/{slug}/` and
`.agents/`. All lifecycle state lives in **files** (`TECH.md` frontmatter, `META.md`), re-read fresh
each invocation — nothing relies on Claude-specific memory. The scripts are invoked by repo-relative
path (`.agents/factory/bin/…`), not a Claude-specific variable.

**One environment assumption is not portable and is called out where it appears:** `del` (reversible
trash) is used instead of `rm` in `cm-roadmap`, because `rm` is blocked in the maintainer's
environment. On a harness without `del`, use your own reversible-delete mechanism, or `git rm` for a
tracked file — but never a bare `rm` on a seed, whose deletion is otherwise unrecoverable outside git
history.
...
markers, the invariant gate, blind evidence-based review, the fail-closed sandbox, and
silence-by-default meta-notes all degrade gracefully. When adapting a skill for another harness, keep
instructions imperative and checkable, and preserve every STOP condition — they are the safety net.
Friction you hit doing so is itself a meta-note (`spec/{slug}/META.md`) for `/cm-harness` to fix.
