...
> **Origin spec.** The *what* and *why* — the locked contract `cm-review` grades against.
> The *how* lives in [`PLAN.md`](PLAN.md) and [`TECH.md`](TECH.md) (written by `cm-plan`).
...
Every criterion must be checkable without a real cluster and without a real identity provider — the
review pass drives the CLI through `.agents/factory/bin/sandbox.sh`. If one genuinely is not, mark
it **human-verified** here and say what the human has to run.

## Non-goals (no-gos)

Explicit exclusions that keep scope bounded to the appetite. Naming what we are **not** doing is as
important as what we are. If a non-goal discharges itself by pointing somewhere else ("record it as
a seed", "the harness cycle covers it"), the named destination must actually receive it —
`/cm-roadmap` verifies exactly that before retiring anything.
...
CLARIFICATION: …]` and **block** `cm-plan` — never guess.

- **Q:** <question> — **A:** <answer> (resolved YYYY-MM-DD).

## Related materials

- Issue: <https://github.com/purduercac/rcac-mcp/issues/NN>
- Seed: <`issues/{slug}.md`, when this GOAL was promoted from a recorded deferral — the link stays
  after `/cm-roadmap` retires the seed, so `git log --diff-filter=D` recovers it>
