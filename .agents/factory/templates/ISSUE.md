...
> session does not have to re-derive it. It is **not** graded by `cm-review` and must never be
> copied into `spec/{slug}/GOAL.md` verbatim — `/cm-feature` promotes it, and that is where
...
| `status:` | Means | What `/cm-feature` does with it |
|---|---|---|
...
| `adopted:{slug}` | Promoted; `spec/{slug}/` now owns it. | Nothing. The seed and its ROADMAP entry stay while the cycle is in flight, because it may bounce or be abandoned; `/cm-roadmap` deletes both once the branch lands on `main`. |
...
questions*, apart from the ordered Part cycles, so the index stays an index of *work*. A terminal
record keeps its file, because nothing else in the repository records that the question was ever
asked. The hidden lane's counterpart heading is *Not scheduled* in `.security/ROADMAP.md`.

`shaped` is not a shortcut past the human gate. It records that the *shaping* happened, not that
the work was accepted; acceptance is still `/cm-feature` creating the branch and the GOAL.
...
`.security/issues/` — see the deferral table in [`AGENTS.md`](../AGENTS.md). For this project that
lane is not hypothetical: anything describing a live path to running a command as the wrong user, on
the wrong host, or without the intended authentication belongs there rather than here.
...
- Found by: <slug + phase, e.g. `strip-docs-subsystem` P3 — or an out-of-cycle review>
