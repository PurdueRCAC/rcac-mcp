...
> loop. Written by the lifecycle skills (`cm-feature` / `cm-plan` / `cm-build` / `cm-review`) when the
> **skillset itself** costs something; read by `cm-publish` (surfaced in the PR) and applied by
> `/cm-harness`. This file is **orthogonal** to the `GOAL → PLAN → TECH → REVIEW` spine — it is about
...
> (that belongs in `GOAL.md` / `REVIEW.md`) is **not** a finding. The blind `cm-review` correctness
...
Brief, optional reinforcement: a part of a skill / the harness that materially helped, so `/cm-harness`
...
spec/{slug}/META.md`). Skills always write `status=open`; only `/cm-harness` flips it. `target` is a
...
`| applied | rejected | deferred` (written by `/cm-harness`).
