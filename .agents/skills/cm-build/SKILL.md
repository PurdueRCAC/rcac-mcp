...
name: cm-build
description: >-
  Resume and execute a cluster-mcp feature's phased roadmap. Discovers spec/{slug}/TECH.md from the
...
allowed-tools: Read, Write, Edit, Grep, Glob, Bash(git status *), Bash(git branch *), Bash(git rev-parse *), Bash(git log *), Bash(git diff *), Bash(git add *), Bash(git commit *), Bash(uv run *), Bash(uv sync *), Bash(.agents/factory/bin/sandbox.sh *), Bash(head *)
---

# cm-build — execute the roadmap (resume-and-implement)

## When to Use

Invoke `/cm-build` on a feature/fix branch whose `spec/{slug}/TECH.md` exists, to execute the next
...
- **Verify by driving the CLI, not just tests — and always in the sandbox.** Run the phase's
  `verify:` command; for behavior, exercise the real flow through
  `.agents/factory/bin/sandbox.sh sh -c "…"`, which gives the run a throwaway `HOME`, blanks the auth
  environment, and unsets `RCAC_SSH_HOST` so the default stdio→`ssh` path fails closed. **Never point
  a verify drive at a real cluster** — not with `--ssh-host`, not with an inherited `RCAC_SSH_HOST`.
  **Exit 0 is necessary but not sufficient** — assert a concrete post-condition (a tool present in
...
- **Honor `AGENTS.md`**: code conventions (incl. declarative comments — no spec `R#`/`P#` ids in
  source), the executor/ContextVar contract, the `cmdkit.app.exit_status` constants, and the
  same-commit rule (a tool change updates `SERVER_INSTRUCTIONS` **and** the `README.md` inventory).
  Consult `invariants.md` for the footguns the phase touches.
- **Circuit breaker (durable).** Every red verify gate is recorded on file via
  ...
  builds (unknowns unresolved), **stop-and-re-shape**: STOP and recommend `/cm-plan` (or human
  input) rather than looping. Respect the appetite.
- **`main` is off-limits; never push, squash, force-push, or open PRs** — that is `/cm-publish`.
...
3. **Remediation mode.** If the FSM shows `top_status: blocked` or `review.verdict:
   changes-requested`, a prior `/cm-review` requested changes: read `spec/{slug}/REVIEW.md`, then make
...
(`uv run python -c "import rcac_mcp"`; drive the CLI in the sandbox for behavior). If implementation
reveals a real correction to `TECH.md`/`PLAN.md`, amend `TECH.md` (Step 5 rules); if it reveals a
`GOAL.md` contradiction, STOP and escalate.

### Step 4 — Verify gate
Run the phase's `verify:` command (plus any CLI drive, in the sandbox). "Green" means the **asserted
post-condition held** — the observed output is correct — not merely that the command exited 0. Green →
proceed. Red → STOP; do not mark done or advance state; record the failure
...
misleadingly (e.g. "exit 0" hid the wrong behavior); an allowed-tools/step mismatch. **Stay silent
...
(recurrence across phases is exactly the signal `/cm-harness` acts on); a fix that would weaken a
non-negotiable gate (tests, the sandboxed CLI verify, an `invariants.md` item) is `severity=high` and
must say so. **Records only** — `/cm-harness` applies fixes later, human-reviewed. Use the next unused
`F#`; always write `status=open`; append the finding as a section **outside** any code fence:
```markdown
## F<n> — <one-line title>
`origin=cm-build:{id} severity=<high|medium|low> category=<instruction|steering|tooling|template|missing-guidance> status=open target=<best-guess file>`
...
```
`cm-build` is **the richest source** and runs **per phase across separate invocations** — appending to
...
output — the same house style as `cm-feature`/`cm-plan`. There is **no `WIP:` prefix**: every branch
commit is squashed into the single PR-title commit at `cm-publish`, so subjects only need to read well
in the PR's commits tab. For a remediation commit (a phase reopened by `cm-review`), keep the `{id}`
and describe the fix, e.g. `[fix] Build {slug} P1: F1 — key the context cache per identity (R6)`. Body
only for non-obvious decisions or to record a `TECH.md` amendment. **No co-author trailer.** Do not
push. Before committing, confirm `git status` shows nothing stray from a sandbox drive — the sandbox
`cd`s out of the repo precisely so `git add -A` cannot sweep a scratch file into the commit.
...
the FSM is fully `done` (`status: in_review`), recommend a **clean-session** `/cm-review`.

## Examples

- `/cm-build` — next incomplete phase, run its verify, one clean `[category]` commit, stop + report.
- `/cm-build status` — FSM state + last commit; no work.
- `/cm-build dry run phase P4` — report what P4 would do (items, files, verify, commit); no edits.
- `/cm-build through P3` — run each incomplete phase up to P3, one commit each, stopping at boundaries.
...
- This skill never ships to `main`, squashes, or force-pushes — that's `/cm-publish`. Out
  of scope (`merge`, `push`, `open PR`, `bump version`) → STOP and point at `/cm-publish`.
