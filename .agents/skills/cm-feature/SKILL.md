...
name: cm-feature
description: >-
  Start a new cluster-mcp feature/fix/refactor from a clean main branch. Safety-checks the tree,
...
# cm-feature — shape the goal

## When to Use

Invoke `/cm-feature` on a clean `main` to begin a new unit of work. It produces exactly one
artifact — a refined `spec/{slug}/GOAL.md` on a fresh branch — and stops for your sign-off before the
expensive `/cm-plan` step. This is **shaping**, in the Shape Up sense: make the goal coherent,
...
- `appetite small` / `appetite big` → set appetite; else default `small` for `kind: fix`, `big` for
  ...
  down fails `cm-review`'s scope check against a contract a human already accepted. Record the
...
- **Branch mapping:** `kind: fix` → `fix/{slug}`; every other kind → `feature/{slug}`. The `kind:`
  ...
  defined branch. `harness/{slug}` is reserved for `/cm-harness` and is never created here.
...
- **Shaping only.** No research fan-out, no broad code exploration, no implementation. If you feel
  the urge to research, that is `/cm-plan`'s job.
...
- **A criterion must be checkable without a real cluster.** `cm-review` grades by driving the CLI
  through `.agents/factory/bin/sandbox.sh`, which cannot reach one. Phrase criteria against what that
  sandbox can observe; where a criterion genuinely needs root, a sudoers rule, two real identities, or
  a live cluster, mark it **human-verified** in the GOAL and say what the human must run. An
  unmarkable criterion is one `cm-review` will silently fail to grade.
...
never the suspected cause/mechanism of the bug, which is unverified until `/cm-plan` root-causes it.** A
...
- **`unshaped`** — nobody has agreed an appetite, non-goals, or a final contract. That negotiation
  is this step's job, and skipping it hands `cm-review` a contract no human ever accepted. Carry the
...
- **`shaped`** — the shaping conversation already happened with a human: dated clarifications, an
  ...
  against current `main`, cite anything that has drifted since it was written, surface that for
...
`/cm-roadmap` retires both once the branch lands on `main`.
...
never republishes an attack mechanism for a weakness that is still live. On this project that means:
name the property being restored ("a tool call executes only as the identity that authenticated it"),
never the path by which it currently is not.
...
that would weaken a non-negotiable gate (tests, the sandboxed CLI verify, an `invariants.md` item) is
`severity=high` and must say so. **Records only** — `/cm-harness` applies fixes later, human-reviewed.
Use the next unused `F#`; always write `status=open`; append the finding as a section **outside** any
code fence:
```markdown
## F<n> — <one-line title>
`origin=cm-feature:<step> severity=<high|medium|low> category=<instruction|steering|tooling|template|missing-guidance> status=open target=<best-guess file>`
...
```
`cm-feature` is shaping-only, so findings here are usually about ambiguous shaping guidance or the
`GOAL.md` template.
...
Report: branch, slug, kind, appetite, the R-ID list, any open clarifications, and any criterion
marked human-verified. Tell the human the sign-off gate: review `spec/{slug}/GOAL.md`, then run
**`/cm-plan`** to research + design. Stop.

## Examples

- `/cm-feature add a --dry-run flag to sbatch that prints the script without submitting` — infer
  `feature`, derive slug `sbatch-dry-run`, create `feature/sbatch-dry-run`, shape the GOAL.
- `/cm-feature spec/split-slurm-extra/GOAL.md` — adopt the hand-written GOAL, slug
  `split-slurm-extra`, branch `feature/split-slurm-extra`, refine in place.
- `/cm-feature fix the rcac://storage resource raises TypeError because storage_paths is a Tool` —
  `kind: fix`, appetite small, branch `fix/{slug}`.
- `/cm-feature issues/strip-docs-subsystem.md` — promote the recorded deferral: shape its draft
  R-IDs into a contract, then flip the issue to `status: adopted:strip-docs-subsystem`.

## Notes

- This skill never researches, edits source, or pushes. That's `/cm-plan`, `/cm-build`, `/cm-publish`.
- If a requirement can't be made unambiguous with the human right now, leave the `[NEEDS
  CLARIFICATION]` marker in place and STOP — an ambiguous GOAL blocks `/cm-plan`.
